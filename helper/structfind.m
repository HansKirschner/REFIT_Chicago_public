...
% else
%     index=find(arrayfun(@(x)(cmp(x,field,value)),a,'uniformoutput',true));return;
% end

%%
%Simple addition: If input is a cell array of strings, loop through fields 
K = 1; clear index
if iscell(value) && length(value)>1
    for E = 1 : length(value)
        try index(K) = find(strcmpi(a.(field),value{E})==1); K = K + 1;
        catch 
            if ~isempty(find(strcmpi({a.(field)},value{E})==1))
                index(K) = find(strcmpi({a.(field)},value{E})==1); K = K + 1;
            end
        end
    end
elseif iscell(value) && length(value) == 1
    index = find(strcmp({a.(field)},value{1})); 
else
    index = [];
end
return
