function input = getBehavModelInput_REFITChicagoRWPH_DrugS(datastruct,paramsToFit,paramValue)
%keyboard
...
for p = 1:length(paramsToFit)
    ...
    elseif strcmp(paramsToFit{p},'FeedConfrim')
        ...
        input.lb(:,p) = -100;
        input.ub(:,p) = 100;
        input.newStartPoint(p) = gamrnd(5, 5);
        input.priorPdf(p) = pdf('Gamma', paramValue, 5, 5);
