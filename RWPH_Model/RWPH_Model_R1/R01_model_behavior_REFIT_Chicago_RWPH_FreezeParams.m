...
allParamSets = {paramset1,paramset2,paramset3,paramset4};
...
for tt = 1:length(allParamSets)
    ...
    for paramSet = 1:size(allParamsToFit,1)
        ...
        saveFileDir = fullfile(fminconDir,sprintf('PL_FP_wGenPrior_fminconResult_%s.mat',paramStr));
        saveFileDirData = fullfile(savepath,sprintf('PL_FP_wGenPrior_Data_%s.mat',paramStr));

        saveTempFileDir     = fullfile(fminconDir,sprintf('PL_FP_wGenPrior_fminconResult_%s_incomplete.mat',paramStr));
        saveTempFileDirData = fullfile(savepath,sprintf('PL_FP_wGenPrior_Data_%s_incomplete.mat',paramStr));
        runThisSub = false;
        ...
        if runThisSub
            for s = s_start:length(dstruct)
                ...
                input = getBehavModelInput_REFITChicagoRWPH_FP(dstruct(s),allParams(thisParamsBin),nan,s);
                ...
                while i <= reps
                    ...
                    newInput = getBehavModelInput_REFITChicagoRWPH_FP(dstruct(s),allParams(thisParamsBin),nan,s);
