...
for tt = 1:length(allParamSets)
    ...
    % Load data
    load('data/all_behavior_Moca.mat');
    ...
    for paramSet = 1:size(allParamsToFit,1)
        ...
        saveFileDir = fullfile(fminconDir,sprintf('Moca_wGenPrior_fminconResult_%s.mat',paramStr));
        saveFileDirData = fullfile(savepath,sprintf('Moca_wGenPrior_Data_%s.mat',paramStr));

        saveTempFileDir     = fullfile(fminconDir,sprintf('Moca_wGenPrior_fminconResult_%s_incomplete.mat',paramStr));
        saveTempFileDirData = fullfile(savepath,sprintf('Moca_wGenPrior_Data_%s_incomplete.mat',paramStr));
        runThisSub = false;
...
%% Check fit
clear
load('data/Moca_wGenPrior_Data_111110.mat')
...
figure(1);clf;
shadedErrBar([], AGF_running_average(mean(choice,2),2,2), AGF_running_average(se(choice',0.99),2,2),'b'); hold on;
plot(AGF_running_average(mean(ChoiceProb,2),2,2),'linewidth',3,'Color','m');
plot(mean(Prob,2)/100,'LineWidth',2,'Color','k')
xlim([1 length(choice)])
set(gca,'xtick',[])
legend({'choice' 'GT' 'CP'},'FontSize',14);
set(0,'DefaultLegendAutoUpdate','off')
ylabel('P(choice)','FontSize',14, 'FontWeight', 'bold')
gridxy(find([ProbChange(:,1)==1]))
