...
for tt = 1:length(allParamSets)
    ...
    for paramSet = 1:size(allParamsToFit,1)
        ...
        saveFileDir = fullfile(fminconDir,sprintf('Session2_wGenPrior_fminconResult_%s.mat',paramStr));
        saveFileDirData = fullfile(savepath,sprintf('Session2_wGenPrior_Data_%s.mat',paramStr));

        saveTempFileDir     = fullfile(fminconDir,sprintf('Session2_wGenPrior_fminconResult_%s_incomplete.mat',paramStr));
        saveTempFileDirData = fullfile(savepath,sprintf('Session2_wGenPrior_Data_%s_incomplete.mat',paramStr));
        runThisSub = false;
