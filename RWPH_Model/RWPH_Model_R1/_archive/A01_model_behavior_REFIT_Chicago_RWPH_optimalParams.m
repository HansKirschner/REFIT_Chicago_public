...
for tt = 1:length(allParamSets)
    ...
    for paramSet = 1:size(allParamsToFit,1)
        ...
        saveFileDir = fullfile(fminconDir,sprintf('optfminconResult_V2_%s.mat',paramStr));
        saveFileDirData = fullfile(savepath,sprintf('optData_V2_%s.mat',paramStr));

        saveTempFileDir     = fullfile(fminconDir,sprintf('optfminconResult_%s_V2_incomplete.mat',paramStr));
        saveTempFileDirData = fullfile(savepath,sprintf('optData_V2_%s_incomplete.mat',paramStr));
        runThisSub = false;
        ...
        if runThisSub
            for s = s_start:length(dstruct)
                ...
                input = getBehavModelInput_REFITChicagoRWPH_optParams(dstruct(s),allParams(thisParamsBin),nan);
                ...
                while i <= reps
                    ...
                    try
                        [fminconOutput]= fitLearningFunc_REFITChicagoRWPH_opt(input);
                    catch
                    ...
                    newInput = getBehavModelInput_REFITChicagoRWPH_optParams(dstruct(s),allParams(thisParamsBin),nan);
