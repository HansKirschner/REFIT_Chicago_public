...
for tt = 1:length(allParamSets)
    ...
    for paramSet = 1:size(allParamsToFit,1)
        ...
        saveFileDir = fullfile(fminconDir,sprintf('fminconResult_%s.mat',paramStr));
        saveFileDirData = fullfile(savepath,sprintf('Data_%s.mat',paramStr));

        saveTempFileDir     = fullfile(fminconDir,sprintf('fminconResult_%s_incomplete.mat',paramStr));
        saveTempFileDirData = fullfile(savepath,sprintf('Data_%s_incomplete.mat',paramStr));
        runThisSub = false;
        ...
        if runThisSub
            for s = s_start:length(dstruct)
                ...
                input = getBehavModelInput_REFITChicagoRWPH_noP(dstruct(s),allParams(thisParamsBin),nan);
                ...
                while i <= reps
                    ...
                    newInput = getBehavModelInput_REFITChicagoRWPH_noP(dstruct(s),allParams(thisParamsBin),nan);
