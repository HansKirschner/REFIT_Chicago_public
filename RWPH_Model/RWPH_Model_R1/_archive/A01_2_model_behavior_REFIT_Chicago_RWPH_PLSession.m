...
for tt = 1:length(allParamSets)
    ...
    for paramSet = 1:size(allParamsToFit,1)
        ...
        saveFileDir = fullfile(fminconDir,sprintf('PLS_wGenPriors_fminconResult_%s.mat',paramStr));
        saveFileDirData = fullfile(savepath,sprintf('PLS_wGenPriors_Data_%s.mat',paramStr));

        saveTempFileDir     = fullfile(fminconDir,sprintf('PLS_wGenPriors_fminconResult_%s_incomplete.mat',paramStr));
        saveTempFileDirData = fullfile(savepath,sprintf('PLS_wGenPriors_Data_%s_incomplete.mat',paramStr));
        runThisSub = false;
