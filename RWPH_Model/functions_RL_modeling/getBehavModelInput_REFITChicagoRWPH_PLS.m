function input = getBehavModelInput_REFITChicagoRWPH_PLS(datastruct,paramsToFit,paramValue)
%keyboard
...
for p = 1:length(paramsToFit)
    ...
    elseif strcmp(paramsToFit{p},'FeedConfrim')
        ...
        input.lb(:,p) = -100;
        input.ub(:,p) = 100;
        input.newStartPoint(p) = gamrnd(3.5, 5);
        input.priorPdf(p) = pdf('Gamma', paramValue, 3.5, 5);
