...
loadpath = 'rawData_DDP_Reversal_Learning_Task/';

% read data names
files = [dir([loadpath 'DDP*.O-REFIT3.dat']); dir([loadpath 'DDP*.o-REFIT4.dat']); dir([loadpath 'DDP*.o-REFIT5.dat']) ...
    ; dir([loadpath 'DDP*.o-REFIT6.dat'])];
...
%deal with probelm files - VP 857 has < 200 trials
VPN([76 89 94 95])          = [];
VPN_full([76 89 94 95])     = [];
...
for a = 1:size(VPN,1) %loop over participants and extract relevant info
    ...
    EndPOints_O(a) = rd.Bonus_sum(end);

end


ert
%save('Data/all_behavior_Orientation','D');
%save('Data/all_EndPOints_Orientation','EndPOints_O');

%outlier detection
[~,~,~,~,stats,outlier,outlier_num] = grubbs(Misses, VP(1,:), 0, 0.05,0);

disp(['Grubbs test for outlier detection with a one sided alpha of 0.01 detected: ' num2str(length(outlier_num)) ' outlier.']);
